...
<h1 class="bg-success" style="padding: .5em 1em; margin-bottom: 1.5em">
    Φάση Γ - Αναθέσεις Ιστότοπου σε Αξιολογητές Β
</h1>
<div style="padding: 0 2em;">


    <p style="font-size: 1.5em">
        <a href="{{ route('sites_grades_c') }}">&larr; Επιστροφή στις Αναθέσεις Γ</a>
    </p>
    ...
    <div class="row">
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            @foreach(App\Evaluation_c::where('site_id', $site->id)->get() as $evaluation)
                                <?php $grader = App\Grader::find($evaluation->grader_id); ?>

                                <?php $assigned_grader_ids[] = $grader->id; ?>

                                <?php $assigned_sites = App\Evaluation_b::where('grader_id', $grader->id)->count(); ?>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @endif
                                        <p>
                                            <strong>Αναθέσεις Φάσης Γ:</strong> {{ App\Evaluation_c::where('grader_id', $grader->id)->count() }}
                                        </p>
                                        <p <?php echo $the_bg_2; ?>>
                                            <strong>Αναθέσεις Φάσης Β:</strong> {{ App\Evaluation_b::where('grader_id', $grader->id)->count() }}
                                        </p>
                                        ...
                                        <p>
                                            <a class="btn btn-info" href="{{ route('send_extra_c_to_grader_b', [$evaluation->id]) }}" role="button" onclick="return confirm('Εϊστε σίγουρος;');">
                                                Αποστολή email για την Ανάθεση (ΝΕΑ - Αξιολογητής Γ)
                                            </a>
                                        </p>
                                        <p>
                                            <a class="btn btn-warning" href="{{ route('send_extra_c_to_grader_b_20pc', [$evaluation->id]) }}" role="button" onclick="return confirm('Εϊστε σίγουρος;');">
        ...
        <div class="row">
            <div class="col-md-12">
                {!! Form::open(['route' => 'evaluations.store_manual_c', 'class' => 'form-horizontal', 'role' => 'form', 'data-parsley-validate']) !!}
                ...
                <div class="form-group">
                    <div class="col-md-12">
                        ...
                        <p class="help-block">
                            ...
                            <strong>Φάση Α/Β/Γ</strong> - Αναθέσεις Φάσης Α/Β/Γ<br>                            
                        </p>
                                ...
                                @if($mygrader->user->hasRole('grader_b'))
                                    ...
                                    @if($mygrader->id != $site->grader_id && $mygrader->district_id != $site->district_id && $grader_cat_id != $site->cat_id && !in_array($mygrader->id, $assigned_grader_ids))
                                        <option value="{{ $mygrader->id }}">
                                            ...
                                            Φάση Β: {{ App\Evaluation_b::where('grader_id', $mygrader->id)->count() }}                                         
                                            Φάση Γ: {{ App\Evaluation_c::where('grader_id', $mygrader->id)->count() }}                                         
        ...
        <p style="font-size: 1.5em">
            <a href="{{ route('sites_grades_c') }}">&larr; Επιστροφή στις Αναθέσεις Β</a>
        </p>
